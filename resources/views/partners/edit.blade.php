    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <p class="text-muted mb-1"><a href="{{ route('partners.index') }}"
                    class="text-decoration-none text-muted">Partenaires</a> / Édition</p>
            <h1 class="h3 fw-bold">Modifier {{ $partner->name }}</h1>
        </div>
    </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                            <option value="veterinarian" {{ old('type', $partner->type) == 'veterinarian' ? 'selected' : '' }}>Vétérinaire</option>
                            <option value="pet_store" {{ old('type', $partner->type) == 'pet_store' ? 'selected' : '' }}>Animalerie</option>
                            <option value="shelter" {{ old('type', $partner->type) == 'shelter' ? 'selected' : '' }}>Refuge</option>
                            <option value="supplier" {{ old('type', $partner->type) == 'supplier' ? 'selected' : '' }}>Fournisseur</option>
                            <option value="other" {{ old('type', $partner->type) == 'other' ? 'selected' : '' }}>Autre</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('contact_person') is-invalid @enderror" id="contact_person"
                            name="contact_person" value="{{ old('contact_person', $partner->contact_person) }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="url" class="form-control @error('website') is-invalid @enderror" id="website"
                            name="website" value="{{ old('website', $partner->website) }}" placeholder="https://...">
                        @error('website')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control @error('discount_rate') is-invalid @enderror" id="discount_rate"
                            name="discount_rate" value="{{ old('discount_rate', $partner->discount_rate) }}" min="0" max="100" step="0.01">
                        @error('discount_rate')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('services') is-invalid @enderror" id="services" name="services"
                            rows="3">{{ old('services', $partner->services) }}</textarea>
                        @error('services')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes"
                            rows="3">{{ old('notes', $partner->notes) }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
